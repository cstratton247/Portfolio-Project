...
private int height;
Reasoning:
BiomeMap is a 2D grid, which is represented by this structure, with width and height being x and y. 
Biome is an enum, so that fixes the issue of null occuring unless intended and can be checked with ==.
It is easily representable due to its simplicity.
